...
# CONFIG
# ========================
parser = argparse.ArgumentParser(description="Temporal SCMVC on WISDM with Local Contrastive Loss")
...
parser.add_argument("--local_chunk_size", default=50, type=int)
...
def split_into_local_chunks(xs, chunk_size):
    """
    Split each view tensor into smaller temporal chunks.

    xs:
        list of tensors, each [batch, time, features]
    ...
    Returns:
        chunks:
            ...
            Example:
            [
              [view1_chunk1, view2_chunk1, view3_chunk1],
              [view1_chunk2, view2_chunk2, view3_chunk2],
              ...
            ]
    """
...
def compute_local_contrastive_loss(model, xs, contrastive_loss_fn, view, chunk_size):
    """
    Compute local window-level contrastive loss.

    For each local temporal chunk:
    - encode the chunk using the same temporal model
    - compute fused representation H_local
    - compute view-specific common representations r_local
    - apply SCMVC-style contrastive loss between H_local and each r_local

    The final local loss is the average over chunks.
    """
...
for epoch in range(1, args.con_epochs + 1):
    ...
    if acc > best_acc:
        ...
        torch.save(model.state_dict(), "./models/wisdm_temporal_local_best.pth")
...
print("Best model saved to: ./models/wisdm_temporal_local_best.pth")